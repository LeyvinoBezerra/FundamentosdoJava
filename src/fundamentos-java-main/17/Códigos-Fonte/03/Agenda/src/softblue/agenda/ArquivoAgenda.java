...
/**
 * Classe que manipula o arquivo onde os registros da agenda s�o armazenados
 */
public class ArquivoAgenda {
	...
	public void gravar(Collection<Contato> contatos) throws IOException {
		...
		try {
			// Utiliza um PrintWriter para gravar as informa��es
			writer = new PrintWriter(ARQUIVO_AGENDA);
			
			for (Contato contato : contatos) {
				// Escreve cada contato em uma linha do arquivo no padr�o '<nome>,<telefone>'
	...
	public List<Contato> ler() {
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				// Divide cada linha do arquivo em tokens, onde o caractere ',' � o delimitador
			...
			// Se o arquivo de contatos n�o for encontrado, retorna a lista vazia
			return contatos;
